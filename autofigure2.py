...
# ============================================================================
# 步骤三：裁切 + rembg 去背景 (CPU, ONNX, 无需 GPU)
# ============================================================================

def _rembg_remove_background(image: Image.Image, output_path: Path) -> str:
    """使用 rembg (u2net, ONNX CPU) 去除背景并保存"""
...
def crop_and_remove_background(
    ...
    """
    根据 boxlib.json 裁切图片并使用 rembg 去背景

    文件命名使用 label: icon_AF01.png, icon_AF01_nobg.png
    """
    print("\n" + "=" * 60)
    print("步骤三：裁切 + rembg 去背景 (CPU)")
    print("=" * 60)
    ...
    print(f"使用 rembg (ONNX CPU) 进行背景去除，首次运行会自动下载 u2net 模型...")
